...
export const getAllAuctionItems = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/v1/auctionitem/allitems`,
...
export const getMyAuctionItems = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/v1/auctionitem/myitems`,
      { withCredentials: true }
    );
...
export const createAuction = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/v1/auctionitem/create`,
      data,
      {
        withCredentials: true,
